...
public class ProtocolClient extends GameConnectionClient {
    ...
    private void updateGhostNPC(Vector3f position, double gsize) {
        ...
        ghostNPC.setLocalLocation(position);
        if (gsize == 1.0)
            gs = false;
        else
            gs = true;
    ...
    protected void processPacket(Object message) {
        ...
        if (msgTokens.length > 0) {
            ...
            if (msgTokens[0].compareTo("createNPC") == 0) { // create a new ghost NPC
            ...
            if (msgTokens[0].compareTo("isnear") == 0) {
                // compare distance between npc and avatar, if below criteria, send packet with
                // message isnear
                System.out.println("checking if avatar is near npc");
                Vector3f ghostPosition = new Vector3f(
                        Float.parseFloat(msgTokens[1]),
                        Float.parseFloat(msgTokens[2]),
                        Float.parseFloat(msgTokens[3]));
                if (ghostPosition.distance(game.getAvatar().getWorldLocation()) < Float.parseFloat(msgTokens[4])) {
                    try {
                        System.out.println("SENDING TO SERVER THAT AVATAR IS NEAR");
                        String nearMessage = "isnear";
                        nearMessage += "," + id;
                        sendPacket(nearMessage);
                        ghostNPC.setSize(false);
                    } catch (IOException e) {
            ...
            if (msgTokens[0].compareTo("mnpc") == 0) {
                Vector3f ghostPosition = new Vector3f(
                        ...
                        Float.parseFloat(msgTokens[2]),
                        Float.parseFloat(msgTokens[3]));
                System.out.println("updated ghost npc position");
                updateGhostNPC(ghostPosition, 0);
     ...
     * Returns true if we're connected or still trying, false if all attempts
     * failed.
     */
